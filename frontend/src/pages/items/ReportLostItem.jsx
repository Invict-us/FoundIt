...
import {
  ...
  MapPin, Calendar, Phone, Mail, Tag, FileText,
  CheckCircle2, Loader2, ArrowLeft, Zap, Search
} from 'lucide-react';
...
export default function ReportLostItem() {
  ...
  const validate = () => {
    ...
    if (!form.dateLost) newErrors.dateLost = 'Date lost is required';
    setErrors(newErrors);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        contact: form.contactPhone || form.contactEmail,
        imagePath
      };
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        ...
        >
          ...
          <p className="text-slate-500 dark:text-slate-400">Your lost item has been reported. Redirecting to dashboard...</p>
        </motion.div>
      </div>
    );
  }
    ...
    >
      ...
      <motion.div variants={itemVariants} className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-[#1E3A5F] dark:text-white">Report a Lost Item</h1>
        </div>
          ...
          <Card glass>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <Input
                label="Description"
                type="textarea"
                placeholder="Describe the item in detail — color, brand, distinguishing marks, contents..."
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <Input
                  label="Room / Area"
                  type="text"
                  placeholder="e.g., Reading Room B"
                  value={form.room}
                  ...
                  <div
                    ...
                    className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all duration-300 ${
                      dragOver
                        ...
                        : 'border-slate-300 dark:border-slate-600 hover:border-[#4A90E2] hover:bg-slate-50 dark:hover:bg-slate-700/50'
                    }`}
                  >
                    <Upload className={`w-10 h-10 mx-auto mb-3 ${dragOver ? 'text-[#14B8A6]' : 'text-slate-400'}`} />
                    <p className="text-sm font-medium text-slate-600 dark:text-slate-300">
                      Drag & drop an image or <span className="text-[#14B8A6]">browse</span>
                    </p>
          ...
          <div className="sticky top-24">
            ...
            {loadingSuggestions ? (
              <div className="flex flex-col items-center justify-center py-12 bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700">
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400">Searching reported found items...</p>
              </div>
                    ...
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 flex items-center gap-1">
                          <Calendar className="w-3 h-3" /> {new Date(item.dateFound || item.createdAt).toLocaleDateString()}
                        </p>
                      </div>
                    </div>
                  </Card>
